...
function resolveNicheKey(data) {
  ...
  if (NICHES[candidate]) return candidate;
  console.warn(`[!] Niche "${candidate}" not found. Falling back to default "${DEFAULT_NICHE}".`);
...
function buildHero() {
  ...
  const heroImage = (entities.images && entities.images.length > 0) ? entities.images[0] : (niche.hero.image || '');
  const visualStyle = heroImage 
    ? ` style="background: linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.7)), url('${esc(heroImage)}'); background-size: cover; background-position: center;"`
    : '';
...
<section class="hero">
  <div class="container hero-grid">
    ...
    <div class="hero-visual"${visualStyle}>
